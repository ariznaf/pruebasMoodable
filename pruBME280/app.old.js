...
/*  
    ...
    trace("buffer: "); traceBuffer(buffer);

    let buf2= new Uint8Array(buffer,1)
    sensor.readBlock(REG.DIG_H2,7,buf2.buffer);
    trace("buf2: "); traceBuffer(buf2);
    trace("buffer: "); traceBuffer(buffer);
    return new DataView(buffer);
*/
    let data= new Uint8Array(8);
    sensor.readBlock(REG.DIG_H1,1,data.buffer);
    let data2= new Uint8Array(7)
    sensor.readBlock(REG.DIG_H2,7,data2.buffer);
    for(let i= 0;i<7;i++) data[i+1]=data2[i];
    trace("humcal: "); traceBuffer(data);
...
function calcHum(humcal,raw,t_fine) {
    ...
    humcal.setUint8(5,humcal.getUint8(5)<<4);
    let dig_H4= humcal.getInt16(4,false)>>4;
    let dig_H6= humcal.getUint8(7);
    /*
    trace("dig_H1: ");traceNumber(dig_H1);
    trace("dig_H2: ");traceNumber(dig_H2);
    trace("dig_H3: ");traceNumber(dig_H3);
    trace("dig_H4: ");traceNumber(dig_H4);
    trace("dig_H5: ");traceNumber(dig_H5);
    trace("dig_H6: ");traceNumber(dig_H6);
    */
...
function calcPresInt64(presCal,raw, t_fine)
{
    ...
    var1 = BigInt(t_fine) - 128000n;
    var2 = var1 * var1 * BigInt(dig_P6);
    var2 = var2 + ((var1 * BigInt(dig_P5)) << 17n);
    var2 = var2 + (BigInt(dig_P4) << 35n);
    var1 = ((var1 * var1 * BigInt(dig_P3)) >> 8n) + ((var1 * BigInt(dig_P2)) << 12n);
    var1 = (((1n << 47n) + var1)) * BigInt(dig_P1) >> 33n;
    if (var1 == 0n) return Infinity;   // Don't divide by zero.
    pressure   = 1048576n - BigInt(raw);
    pressure = (((pressure << 31n) - var2) * 3125n)/var1;
    var1 = (BigInt(dig_P9) * (pressure >> 13n) * (pressure >> 13n)) >> 25n;
    var2 = (BigInt(dig_P8) * pressure) >> 19n;
    pressure = ((pressure + var1 + var2) >> 8n) + (BigInt(dig_P7) << 4n);
...
Timer.repeat( id=> {
    ...
    time = Date.now() - time;
    trace("sensor Pressure (int64):"+(press/25600.0).toFixed(5)+' hPa\n time: '+time+'\n');
},1000);
